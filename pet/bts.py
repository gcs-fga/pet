...
import debianbts
from pet.exceptions import BugTrackerException, BinaryNotKnown
...
class _DebianBugReport(_BugReport):
    def __init__(
            self, bugreport,
            binary_source_map, ignore_unknown_binaries=False):
            ...
            if len(parts) == 1:
                ...
                try:
                    ...
                    if parts[0] in self.sources:
                        ...
                        raise BinaryNotKnown(
                            "source for binary {0} is unknown".format(parts[0])
                        )
            else:
                raise BugTrackerException(
                    "cannot parse version '{0}'".format(version))
            for s in sources:
...
class DebianBugTracker(object):
    ...
    def search(self, sources, bug_numbers=None):
        ...
        return [_DebianBugReport(
            b, self.binary_source_map,
